...
def get_april_birthdays(people):
    ...
    for i in range(len(people)):
        if people[i].birthday_month == 'April':
            people[i].birthday()
            april_birthdays.update({people[i].name : people[i].age})
...
def get_most_common_month(people):
    ...
    for i in range(len(people)):
        months[people[i].birthday_month] = months.get(people[i].birthday_month, 0) + 1
        i += 1

    vm = list(months.values())
    km = list(months.keys())

    # TODO: Modify the return statement.
    return km[vm.index(max(vm))]
