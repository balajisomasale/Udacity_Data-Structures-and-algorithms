class Stack:
    ...
    def push(self, data):
        if self.next_index == len(self.arr):
            print("Stack is out of memory. Increasing stack capacity...")
            self._handle_stack_capacity_full()              # Increasing stack capacity, then continuing to push data
    ...
    def _handle_stack_capacity_full(self):
        old_arr = self.arr                                  # keeping a copy to revert back to

        self.arr = [0 for _ in range(2 * len(old_arr))]     # Creating a new stack of double existing length
