...
def list_codes(data):
	...
	for i in calls:
		...
		# Fixed lines
		if i[0].startswith(tel_bangalore):
			my_list_of_calls.append(i[1])

		# Mobile prefixes
		if i[0][:5] == tel_bangalore and i[1][0] == '7':
			my_list_of_calls.append([i[0],i[1]])
		if i[0][:5] == tel_bangalore and i[1][0] == '8':
			my_list_of_calls.append([i[0],i[1]])
		if i[0][:5] == tel_bangalore and i[1][0] == '9':
			my_list_of_calls.append([i[0],i[1]])

		# Tele Marketing
		if i[0][:5] == tel_bangalore and i[1][0:3] == '140':
			my_list_of_calls.append([i[0],i[1]])
...
def list_num(data):
    ...
    for i in data:
    	if i[0][:5] == tel_bangalore:
    		poss_calls.append(i[:2])

    for j in poss_calls:
    	if j[1][:5] == tel_bangalore:
